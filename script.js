  ...
  .then((data) => {
    ...
    const filterEvents = () => {
      ...
      return events.filter((event) => {
        ...
        return endDate >= today && event.CODENAME === eventType && event.GUNAME === eventLocation;
      });
    };
      ...
      filteredEvents.forEach((event) => {
        ...
        properties.forEach((property) => {
          ...
          if (property.key === "ORG_LINK") {
            ...
            if (orgLink.length > 20) { // 일정 길이 이상인 경우
            ...
            propertyElement.innerHTML = `<strong>${property.label}:</strong> ${event[property.key]}`;
          }
